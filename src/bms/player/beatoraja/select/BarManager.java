...
public final class BarManager {
	...
	void init() {
		...
		if(select.main.getIRConnection() != null) {
			bms.player.beatoraja.ir.IRResponse<bms.player.beatoraja.ir.IRTableData[]> response = select.main.getIRConnection().getTableDatas();
...
//		}
		...
		if (false) {
			try {
				bms.player.beatoraja.RandomCourseData stepUpCourse = null;
				...
				try {
					for (RandomFolder randomFolder : randomFolderList) {
						...
						if ((randomFolder.getFilter() != null && randomTargets.length >= 1)
							...
							Bar randomBar = new ExecutableBar(randomTargets, null,
